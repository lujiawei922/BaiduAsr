...
typedef void* (BaiduApi::*Asr)(void *);
...
int asr_online_pushaudio(const char *file_path, const string &push_cmd, bds::BDSpeechSDK *sdk, int thread_seq)
{
...
void  asr_output_callback(bds::BDSSDKMessage &message, void *user_arg)
{
    ...
    const char *time = get_gmt_time().toLatin1().data();
    switch (status) {
...
//            file.close();
        ...
        case bds::EVoiceRecognitionClientWorkStatusError: {
            ...
            fprintf(result_output_file,
                 ...
                 err_desc: %s, sn: %s\n",
                    get_gmt_time().toLatin1().data(), err_code, err_domain, err_desc.c_str(), sn.c_str());
            fflush(result_output_file);
            pthread_mutex_unlock(&thread_mutexes[thread_seq]);
            break;
        }
...
void *asr_thread(void *arg)
{
    ...
    fprintf(stdout, "[%s]thread[%d] finish\n", get_gmt_time().toLatin1().data(), thread_seq);
    fflush(stdout);
    return nullptr;
}

int BaiduApi::sendPcm(QString path)
{
    FILE *err_output_file = stderr;

    for (int i = 0; i < THREAD_NUM; ++i) {
        thread_sequeces[i] = i;
        pthread_mutex_init(&thread_mutexes[i], nullptr);
        strncpy(file_names[i],path.toLatin1().data(), sizeof(file_names[i]));
        // 新线程中开启每个文件的识别
        int32_t ret = pthread_create(&thread_ids[i], nullptr, asr_thread,
    ...
    for (int i = 0; i < THREAD_NUM; ++i) {
        int ret = pthread_join(thread_ids[i], nullptr); // 等待所有线程结束
